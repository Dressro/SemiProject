...
public class RSA_str {

	static final int KEY_SIZE = 2048;
	private static HashMap<String, String> rsaKeyPair = createKeypairAsString();
    ...
    private static HashMap<String, String> createKeypairAsString() {
    ...
    public static String decode(String encryptedData, String stringPrivateKey) {
        ...
        try {
            //평문으로 전달받은 개인키를 개인키 객체로 생성
            KeyFactory keyFactory = KeyFactory.getInstance("RSA");
            byte[] bytePrivateKey = Base64.getDecoder().decode(stringPrivateKey.getBytes());
            PKCS8EncodedKeySpec privateKeySpec = new PKCS8EncodedKeySpec(bytePrivateKey);
            PrivateKey privateKey = keyFactory.generatePrivate(privateKeySpec);

            //만들어진 개인키 객체를 기반으로 암호화모드로 설정
            Cipher cipher = Cipher.getInstance("RSA");
